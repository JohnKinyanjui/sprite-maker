<script lang="ts">
  ...
  onMount(()=>{
    ...
    const shortcuts=(event:KeyboardEvent)=>{
      ...
      const order=selectedWorktree?.kind==="vfx"?["chat","sprites","references","animate","vfx","sheets","packs","play"]:["chat","sprites","references","animate","sheets","packs","play"];
...
{:else}
  <main class="studio">
    {#if sidebarOpen}<WorkspaceSidebar {workspace} {worktrees} selectedWorktreeId={selectedWorktree?.id} conversations={sidebarConversations} selectedConversationId={selectedConversation?.id} {runningConversationIds} onWorktree={chooseWorktree} onNewWorktree={()=>worktreeDialog=true} onConversation={chooseConversation} onNewConversation={newConversation} onRenameConversation={renameChat} onArchiveConversation={archiveChat} onSettings={()=>settingsOpen=true} onHome={goHome} onCollapse={()=>sidebarOpen=false}/>{:else}<div class="rail"><button onclick={()=>sidebarOpen=true} title="Open project navigation"><Menu size={15}/></button><button onclick={()=>{renameValue=workspace?.name??"";workspaceMenu=true}} title="Project options"><MoreHorizontal size={15}/></button></div>{/if}
