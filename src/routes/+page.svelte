<script lang="ts">
  ...
  import { startupStageLabel } from "$lib/startup-stages";
  ...
  import { persistConversationLogs } from "$lib/conversation-log";
  import {
    type ActiveChatRequest, animationFrameAssets, appendAssistantDelta, appendConversationActivity, buildChatContext,
    applyAnimationPolishModeToPrompt, buildFullRedrawPrompt, buildMotionPrompt, buildProviderOptions, buildRigPolishPrompt, chatActivityLines, clearConversationRunningRequest, clearRunningRequest, parallelGenerationsInWorkspace,
  ...
  import {
    persistConversationUiSettings, snapshotConversationUiSettings, type ConversationUiSettings,
  ...
  let bootStage = $state("Avvio di Sprite Studio…");
  ...
  let conversationUiSettingsById = $state<Record<string, ConversationUiSettings>>({});
  ...
  function trackConversationActivity(conversationId:string,lines:string[],level:"info"|"warning"|"error"="info",options?:{category?:string;eventType?:string;requestId?:string;details?:Record<string,unknown>}){
    activityByConversation=appendConversationActivity(activityByConversation,conversationId,lines,level);
    void persistConversationLogs(conversationId,lines,{level,category:options?.category??"frontend",eventType:options?.eventType??"activity",requestId:options?.requestId,details:options?.details});
  }
  ...
  function resetGenerationSessions(){
    ...
    conversationUiSettingsById={};
  }
  function rememberConversationUiSettings(conversationId:string){
    conversationUiSettingsById={
      ...conversationUiSettingsById,
      [conversationId]:snapshotConversationUiSettings(conversationStyle,conversationAnimationMode,generationProfile),
    };
  }
  function applyConversationUiSettings(settings:ConversationUiSettings,provider?:ProviderStatus){
    conversationStyle=settings.style;
    conversationAnimationMode=settings.animationMode;
    generationProfile=normalizeGenerationProfile(settings.profile,provider?.modes??[],provider?.id);
  }
  async function flushConversationUiSettings(conversation:Conversation){
    const cached=conversationUiSettingsById[conversation.id]??snapshotConversationUiSettings(conversationStyle,conversationAnimationMode,generationProfile);
    const provider=providers.find(item=>item.id===conversation.provider);
    const profile=await persistConversationUiSettings(conversation,cached,provider?.modes??[]);
    conversationUiSettingsById={...conversationUiSettingsById,[conversation.id]:{...cached,profile}};
  }
  async function loadWorkspaces() {
    workspaces=(await api.loadSidebarState()).workspaces;
  }
  async function loadWorkspace(selected:Workspace,preloaded?:SidebarSnapshot,onBootStage?:(stage:string)=>void) {
    ...
    try {
      ...
      const result=await fetchWorkspaceLoad(selected,preloaded,customArts,providers,()=>selection!==workspaceSelection,stage=>{
        loadStage=stage;
        onBootStage?.(startupStageLabel(stage));
      });
      if(!result)return;
      ...
      if(result.selectedConversation)rememberConversationUiSettings(result.selectedConversation.id);
      onBootStage?.(startupStageLabel("syncing generations"));
      ...
      onBootStage?.(startupStageLabel("restoring chat preview"));
  ...
  async function initialLoad() {
    const providerDetection = api.detectProviders()
      .then(value => { providers = value; })
      .catch(error => notify(`Provider detection could not finish: ${errorMessage(error)}`, "error"));
    try {
      bootStage="Caricamento impostazioni…";
      ...
      workspaces=shell.snapshot.workspaces;
      if(shell.recent){
        bootStage="Apertura progetto…";
        await loadWorkspace(shell.recent,shell.snapshot,stage=>bootStage=stage);
      }
    } catch(error){notify(errorMessage(error),"error");}
    bootStage=startupStageLabel("detecting providers");
    await providerDetection;
    loading=false;
  }
  ...
  async function chooseConversation(conversation:Conversation){
    const outgoing=selectedConversation;
    if(outgoing&&outgoing.id!==conversation.id){
      rememberConversationUiSettings(outgoing.id);
      void flushConversationUiSettings(outgoing);
    }
    ...
    activeTab="chat";
    if(!workspace)return;
    const conversationProvider=providers.find(provider=>provider.id===conversation.provider);
    const cached=conversationUiSettingsById[conversation.id];
    applyConversationUiSettings(cached??{
      style:"inherit",
      animationMode:"rig",
      profile:normalizeGenerationProfile(null,conversationProvider?.modes??[],conversation.provider),
    ...
    const session=await fetchConversationSession(conversation,customArts,conversationProvider);if(selection!==conversationSelection)return;
    messages=session.messages;
    applyConversationUiSettings({style:session.style,animationMode:session.animationMode,profile:session.profile},conversationProvider);
    activeReferenceIds=session.referenceIds.filter(id=>references.some(reference=>reference.id===id));
    const focus=await restoreConversationFocus(conversation,references,activeReferenceIds);if(selection!==conversationSelection)return;
    focusedReferenceId=focus.focusedReferenceId;
    activeReferenceIds=focus.activeReferenceIds;
    rememberConversationUiSettings(conversation.id);
  ...
  async function deleteChat(conversation:Conversation){if(runningRequests[conversation.id]){notify("Stop this chat’s generation before deleting it","error");return;}if(!window.confirm(`Delete "${conversation.title}" permanently? Its messages will be removed and this cannot be undone.`))return;try{await api.deleteConversation(conversation.id);sidebarConversations=sidebarConversations.filter(item=>item.id!==conversation.id);conversations=conversations.filter(item=>item.id!==conversation.id);const nextRunning={...runningRequests};delete nextRunning[conversation.id];runningRequests=nextRunning;const nextActivity={...activityByConversation};delete nextActivity[conversation.id];activityByConversation=nextActivity;if(selectedConversation?.id===conversation.id){const next=conversations[0];if(next)await chooseConversation(next);else await newConversation(selectedWorktree);}notify("Chat deleted");}catch(error){notify(errorMessage(error),"error");throw error;}}
  ...
  async function refineChatPrompt(draft:string){
    if(!selectedConversation||!workspace)throw new Error("Open a project chat before refining a prompt");
    const style=effectiveStyle;const profile=generationProfile;
    return api.refineGenerationPrompt({
      conversationId:selectedConversation.id,
      draft,
      context:buildChatContext({worktree:selectedWorktree,focused:references.find(reference=>reference.id===focusedReferenceId),selectedAsset,styleName:style.name,stylePrompt:style.prompt,customSkills}),
      command:inferChatCommand(draft),
      generation:generationRequestFromProfile(profile),
      model:profile.model||undefined,
      reasoningEffort:profile.reasoningEffort||undefined,
      animationMode:conversationAnimationMode,
      referenceIds:activeReferenceIds,
    });
  }
  async function cancelRefinePrompt(conversationId:string){
    try{await api.cancelRefineGenerationPrompt(conversationId);}
    catch(error){if(studioErrorCode(error)!=="refine_not_running")notify(errorMessage(error),"error");}
  }
  async function handleDebugLogExported(path:string){
    notify("Chat debug log exported");
  ...
  async function send(prompt:string){
    ...
    try{
      ...
      if(shouldRunNativeRigFirst(command,polishMode,animateMaster)){
        ...
        const native=await runNativeRigChatAnimation({
          ...
          onFitWarning:(message)=>{trackConversationActivity(conversation.id,[message],"warning",{category:"rig",eventType:"fit_warning"});notify(message,"error");},
          onActivity:(line)=>{trackConversationActivity(conversation.id,[line], "info",{category:"rig",eventType:"rig_activity"});},
        });
      ...
      if(activity.length)trackConversationActivity(conversation.id,activity,"info",{category:"chat",eventType:"generation_plan"});
    ...
    }catch(error){const next={...runningRequests};delete next[conversation.id];runningRequests=next;const nextControllers={...nativeRigAbortControllers};delete nextControllers[conversation.id];nativeRigAbortControllers=nextControllers;if(error instanceof DOMException&&error.name==="AbortError"){notify("Native rig orchestration cancelled");return;}trackConversationActivity(conversation.id,[errorMessage(error)],"error",{category:"chat",eventType:"send_failed"});notify(errorMessage(error),"error");throw error;}
  }
  async function cancel(){if(!selectedConversation)return;const request=runningRequests[selectedConversation.id];if(!request)return;if(request.id.startsWith("native-rig-")){nativeRigAbortControllers[selectedConversation.id]?.abort();const nextControllers={...nativeRigAbortControllers};delete nextControllers[selectedConversation.id];nativeRigAbortControllers=nextControllers;runningRequests=clearConversationRunningRequest(runningRequests,selectedConversation.id);notify("Native rig orchestration cancelled");messages=await api.listMessages(selectedConversation.id);return;}try{await api.cancelProviderRequest(request.id);}catch(error){notify(errorMessage(error),"error");}}
  async function recoverStaleRunningMessage(conversationId:string){
    if(runningRequests[conversationId])return;
    const refreshed=await api.listMessages(conversationId);
    if(conversationId!==selectedConversation?.id)return;
    messages=refreshed;
    const stillRunning=refreshed.some(message=>message.role==="assistant"&&message.status==="running");
  ...
  async function assetDeleted(){
    selectedAsset=undefined;
    if(!workspace)return;
    assets=await api.scanAssets(workspace.id);
    animations=await api.listAnimations(workspace.id,selectedWorktree?.id);
    rigs=await api.listRigs(workspace.id,selectedWorktree?.id);
    if(selectedWorktree)worktreeAssetIds=await api.listWorktreeAssetIds(selectedWorktree.id);
  }
  ...
  async function changeConversationStyle(value:ConversationStyleId){if(!selectedConversation)return;conversationStyle=value;rememberConversationUiSettings(selectedConversation.id);try{await api.setSetting(`conversation-style:${selectedConversation.id}`,value);notify(value==="inherit"?"Chat now follows the project art direction":`${stylePreset(value,customArts).name} applied to this chat`);}catch(error){notify(errorMessage(error),"error");}}
  async function changeConversationAnimationMode(value:AnimationPolishMode){if(!selectedConversation)return;conversationAnimationMode=value;rememberConversationUiSettings(selectedConversation.id);try{await api.setSetting(`conversation-animation-mode:${selectedConversation.id}`,value);notify(`${animationPolishModeOption(value).label} selected for animations in this chat`);}catch(error){notify(errorMessage(error),"error");}}
  ...
  async function changeGenerationProfile(value:ChatGenerationProfile){if(!selectedConversation)return;generationProfile=normalizeGenerationProfile(value,currentProvider?.modes??[],selectedConversation.provider);rememberConversationUiSettings(selectedConversation.id);try{await api.setSetting(`conversation-generation:${selectedConversation.id}`,generationProfile);}catch(error){notify(errorMessage(error),"error");}}
  async function changeConversationProvider(providerId:string){
    ...
    try{
      ...
      rememberConversationUiSettings(changed.id);
  ...
  $effect(()=>{
    ...
    const staleRunning=conversationId&&messages.some(message=>message.role==="assistant"&&message.status==="running")&&!runningRequests[conversationId];
    if(!staleRunning)return;
    const timer=window.setTimeout(()=>{void recoverStaleRunningMessage(conversationId);},2500);
    ...
    const unlistenPromise=listen<ProviderEvent>("provider-event",async({payload})=>{
      ...
      if(isTerminalProviderEvent(payload.eventType)){
        ...
        if(payload.eventType==="completed"&&request){
          trackConversationActivity(payload.conversationId,["Registering the generated sprite"],"info",{category:"pipeline",eventType:"finalize_started",requestId:payload.requestId});
          try{
            if(request.phase==="master"){
              trackConversationActivity(payload.conversationId,["Master saved — building native rig animation"],"info",{category:"rig",eventType:"master_saved",requestId:payload.requestId});
              ...
              runningRequests={...runningRequests,[payload.conversationId]:{...request,id:nativeRequestId,startedAt:request.startedAt??Date.now()}};
              ...
              let continued: Awaited<ReturnType<typeof continueAfterMasterProviderPhase>> | undefined;
              try{
                continued=await continueAfterMasterProviderPhase({
                  ...
                  onActivity:(line)=>{trackConversationActivity(payload.conversationId,[line],"info",{category:"rig",eventType:"rig_activity",requestId:nativeRequestId});},
                  onFitWarning:(message)=>{trackConversationActivity(payload.conversationId,[message],"warning",{category:"rig",eventType:"fit_warning",requestId:nativeRequestId});notify(message,"error");},
                ...
                runningRequests=clearConversationRunningRequest(runningRequests,payload.conversationId);
                if(error instanceof DOMException&&error.name==="AbortError"){
                  trackConversationActivity(payload.conversationId,["Native rig orchestration cancelled"],"warning",{category:"rig",eventType:"rig_cancelled",requestId:nativeRequestId});
                  notify("Native rig orchestration cancelled");
                }else{
                  trackConversationActivity(payload.conversationId,[`Native rig failed: ${errorMessage(error)}`],"error",{category:"rig",eventType:"rig_failed",requestId:nativeRequestId});
                  throw error;
                }
              }
              if(continued){
                ...
                if(continued.kind==="rig-complete"){
                  assets=continued.completion.assets;rigs=continued.completion.rigs;
                  ...
                  trackConversationActivity(payload.conversationId,[fallbackMessage],"error",{category:"rig",eventType:"rig_fallback",requestId:nativeRequestId,details:continued.details});
                  notify(fallbackMessage,"error");
                }else{
                  trackConversationActivity(payload.conversationId,["Master saved, but native rig animation could not start in the background chat"],"error",{category:"rig",eventType:"rig_fallback",requestId:nativeRequestId,details:continued.details});
                }
                if(!deferProviderClear)runningRequests=clearConversationRunningRequest(runningRequests,payload.conversationId);
              }
            }else{
              await finalizeChatGeneration(request,payload.content);
            }
          }
          catch(error){trackConversationActivity(payload.conversationId,[`Finalize failed: ${errorMessage(error)}`],"error",{category:"pipeline",eventType:"finalize_failed",requestId:payload.requestId});notify(`The provider finished, but the sprite preview could not be refreshed: ${errorMessage(error)}`,"error");}
        }
        if(!deferProviderClear)runningRequests=clearConversationRunningRequest(runningRequests,payload.conversationId);
...
{#if loading}
  <StartupSplash stage={bootStage} />
{:else}
  <main class="studio">
    <ProjectSidebar {workspaces} {workspace} {worktrees} conversations={sidebarConversations} selectedWorktreeId={selectedWorktree?.id} selectedConversationId={selectedConversation?.id} {runningConversationIds} activeView={activePrimary} onView={selectPrimary} onProject={loadWorkspace} onAddProject={()=>projectDialogOpen=true} onConversation={chooseConversation} onNewConversation={newConversation} onRenameConversation={renameChat} onArchiveConversation={archiveChat} onDeleteConversation={deleteChat} onListArchivedConversations={listArchivedChats} onRestoreConversation={restoreArchivedChat} onSettings={()=>settingsOpen=true} onManageProject={()=>{if(workspace){renameValue=workspace.name;workspaceMenu=true;}}}/>
      ...
      <div class="tab-stack">
        ...
        {:else}
          <div class="media-view"><MediaNavigation active={activeTab} counts={{sprites:spriteCount,references:references.length,animate:animations.length,rigs:rigs.length,packs:visiblePacks.length,sessions:sessionCount}} onSelect={selectTab}/><div class="media-content">
            {#if activeTab==="sprites"}<AssetBrowser workspaceId={workspace.id} worktreeId={selectedWorktree?.id} {assets} {animations} packs={visiblePacks} packId={packFilter} selectedAssetId={selectedAsset?.id} onAssets={(value)=>assets=value} onSelect={selectAsset} onOpen={openSpriteGroup} onPack={(value)=>packFilter=value} onLinked={async()=>{if(selectedWorktree)worktreeAssetIds=await api.listWorktreeAssetIds(selectedWorktree.id)}} onDeleted={assetDeleted} onError={(message)=>notify(message,"error")}/>
