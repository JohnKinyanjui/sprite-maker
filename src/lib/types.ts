...
export type ChatGenerationProfile = { profileVersion: number; quality: GenerationQuality; width: number; height: number; frames: number; fps: number; frameMode: FrameMode; minFrames: number; maxFrames: number; allowInterpolation: boolean; allowAutoAdjust: boolean; model: string; reasoningEffort: string };
export type SpriteSlashCommand = "animate" | "sprite" | "character" | "effect" | "pack";
