...
<details class="generation-menu" bind:this={menu}>
  ...
  <div class="popover">
    ...
    <div class="preset-grid" aria-label="Generation quality">
      {#each ["low", "mid", "high", "custom"] as quality}
        <button class:active={profile.quality === quality} onclick={() => chooseQuality(quality as GenerationQuality)}>
          ...
          <span>{quality === "low" ? "32px · Auto" : quality === "mid" ? "64px · Auto" : quality === "high" ? "128px · Auto" : "Your values"}</span>
        </button>
      {/each}
    </div>

    <div class="frame-policy">
      <div><strong>Frames</strong><span>{profile.frameMode === "auto" ? "AI recommends the best count in your range" : "Use an exact count you choose"}</span></div>
    ...
    {:else}
      <div class="fields two auto-range"><label>Minimum<input type="number" min="1" max="32" value={profile.minFrames} onchange={(event) => numberChange("minFrames", event.currentTarget.value)}/></label><label>Maximum<input type="number" min="1" max="32" value={profile.maxFrames} onchange={(event) => numberChange("maxFrames", event.currentTarget.value)}/></label></div>
      <div class="auto-options">
        <button class="switch-row" role="switch" aria-checked={profile.allowAutoAdjust} onclick={()=>commit({...profile,allowAutoAdjust:!profile.allowAutoAdjust})}><span><strong>Frame adjustment</strong><small>Let AI insert or remove planned poses</small></span><i class:on={profile.allowAutoAdjust}><b></b></i></button>
        <button class="switch-row" role="switch" aria-checked={profile.allowInterpolation} onclick={()=>commit({...profile,allowInterpolation:!profile.allowInterpolation})}><span><strong>Interpolation</strong><small>Create deterministic in-between frames</small></span><i class:on={profile.allowInterpolation}><b></b></i></button>
      </div>
    {/if}
      ...
      <div class="help-backdrop" role="presentation" onclick={(event)=>event.target===event.currentTarget&&(showHelp=false)}>
        <div class="help-dialog" role="dialog" aria-modal="true" aria-labelledby="generation-help-title">
          ...
          <dl>
            ...
            <div><dt>Auto or Fixed frames</dt><dd>Auto lets AI recommend the smallest mechanically complete loop inside your range. Fixed uses exactly your chosen count.</dd></div>
            ...
            <div><dt>Frame adjustment</dt><dd>Allows the motion planner to insert or remove poses so anticipation, action, recovery, and loop closure fit.</dd></div>
            <div><dt>Interpolation</dt><dd>Adds deterministic in-between frames after key poses are planned. It is enabled by default; disable it for intentionally stepped or very tiny pixel motion.</dd></div>
