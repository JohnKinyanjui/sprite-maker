<script lang="ts">
  ...
  import { errorMessage, type Animation, type Asset } from "$lib/types";

  let { workspaceId, worktreeId, assets, animations, category, selectedAssetId, onAssets, onSelect, onLinked, onError }: {
    workspaceId: string; worktreeId?: string; assets: Asset[]; animations: Animation[]; category?: string; selectedAssetId?: string;
    onAssets: (assets: Asset[]) => void; onSelect: (asset: Asset) => void; onLinked?: () => void | Promise<void>; onError: (message: string) => void;
  } = $props();
  let search = $state("");
  ...
  let filtered = $derived(groups.filter(group => (activeCategory === "assets" || group.category === activeCategory) && (group.name.toLowerCase().includes(search.toLowerCase()) || group.frames.some(asset => asset.name.toLowerCase().includes(search.toLowerCase())))));
...
<section class="asset-browser">
  ...
  <div class="asset-layout">
    ...
    <div class="content">
      <div class="toolbar"><div class="search"><Search size={13} /><input bind:value={search} placeholder="Filter sprites" /></div><span>Animation frames are grouped into sprite sets</span></div>
      {#if filtered.length}
        <div class="grid">
          {#each filtered as group}
            <button class="asset" class:selected={group.frames.some(asset => selectedAssetId === asset.id)} class:grouped={group.frames.length > 1} onclick={() => onSelect(group.preview)} draggable="true" ondragstart={(event) => drag(event, group.preview)}>
