<script lang="ts">
  ...
  import { Search, Upload, RefreshCw, Image, UserRound, Bug, Mountain, Package, Sparkles, FolderOpen, MoreHorizontal, Trash2 } from "lucide-svelte";
  ...
  let { workspaceId, worktreeId, assets, animations, packs = [], packId = "", category, selectedAssetId, onAssets, onSelect, onOpen, onPack, onLinked, onDeleted, onError }: {
    ...
    onAssets: (assets: Asset[]) => void; onSelect: (asset: Asset) => void; onOpen: (group: SpriteGroup) => void | Promise<void>; onLinked?: () => void | Promise<void>; onDeleted?: () => void | Promise<void>; onError: (message: string) => void;
  ...
  async function removeAsset(asset: Asset) {
    ...
    deletingId = asset.id;
    try {
      if (await confirmAndDeleteAsset(asset)) {
        await onDeleted?.();
      }
    } catch (error) { onError(errorMessage(error)); }
    finally { deletingId = undefined; }
  }
</script>

<section class="asset-browser">
  <header><div><h1>{activePack ? activePack.name : heading}</h1><p>{filtered.length} sprite{filtered.length === 1 ? "" : "s"}{activePack ? " in this pack" : ""} · {assets.length} indexed in project</p></div><div class="header-actions"><button onclick={refresh} title="Scan project files" class:spinning={refreshing}><RefreshCw size={14} /></button><button class="import" onclick={importFile} disabled={importing}><Upload size={13} /> {importing ? "Importing…" : "Import"}</button></div></header>
        ...
        <div class="grid">
          {#each filtered as group}
            <button class="asset" class:selected={group.frames.some(asset => selectedAssetId === asset.id)} class:grouped={group.frames.length > 1} onclick={() => onOpen(group)} draggable="true" ondragstart={(event) => drag(event, group.preview)}>
              ...
              <div class="asset-info"><div><strong>{group.name}</strong><small>{group.preview.width}×{group.preview.height} · {group.frames.length > 1 ? `Sprite set${group.fps ? ` · ${group.fps} FPS` : ""}` : group.preview.format.toUpperCase()}</small></div><div class="asset-actions"><span title="Remove from project" role="button" tabindex="0" class:busy={deletingId===group.preview.id} onclick={(event) => { event.stopPropagation(); void removeAsset(group.preview); }} onkeydown={(event) => event.key === "Enter" && removeAsset(group.preview)}><Trash2 size={13} /></span><span title="Reveal on disk" role="button" tabindex="0" onclick={(event) => { event.stopPropagation(); revealItemInDir(group.preview.path); }} onkeydown={(event) => event.key === "Enter" && revealItemInDir(group.preview.path)}><MoreHorizontal size={14} /></span></div></div>
            </button>
          {/each}
        </div>
      {:else}
