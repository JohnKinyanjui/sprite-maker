<script lang="ts">
  ...
  let alignerSection = $state<"align" | "character" | "production" | "regen">("align");
  let maskTool = $state<"rect" | "brush" | "erase">("rect");
  let maskBrushRadius = $state(8);
  const alignerAlignActive = $derived(alignerOpen && alignerSection === "align");
  const alignerRegenActive = $derived(alignerOpen && alignerSection === "regen");
  ...
  function startNewAnimationDraft() { animationId=undefined;name="New animation";fps=10;looping=true;frames=[];reviewStatus="draft";sizeContract=undefined;contractRetryJob=undefined;alignerOpen=false;alignerSection="align";activeFrame=0;playing=false;qualityOpen=false;qualityReport=undefined;qualityJob=undefined; }
  function toggleAligner() {
    alignerOpen = !alignerOpen;
    if (alignerOpen) alignerSection = "align";
  }
...
<section class="editor">
  ...
  <div class="body" class:with-quality={qualityOpen || alignerOpen || Boolean(exportMetadataPreview)}>
    ...
    <div class="workspace">
      ...
      <div class="preview-area">
        <div class="preview-stage" style={`--guide-baseline:${(guideBaselineY / guideFrameHeight) * 100}%`}>
          {#if currentAsset}
            {#if alignerAlignActive && showAlignGhosts}
            ...
            {#if alignerRegenActive}
              <div class="mask-overlay-wrap">
                {#if maskOverlayMode === "regen"}
                  <FrameMaskOverlay imageSrc={previewSrc(currentAsset.path)} frameWidth={currentAsset.width} frameHeight={currentAsset.height} {scale} mode="regen" showToolbar={false} bind:tool={maskTool} bind:brushRadius={maskBrushRadius} bind:regions={maskRegions} bind:brushStrokes={maskBrushStrokes} bind:regionX bind:regionY bind:regionW bind:regionH />
                {:else}
                  <FrameMaskOverlay imageSrc={previewSrc(currentAsset.path)} frameWidth={currentAsset.width} frameHeight={currentAsset.height} {scale} mode="erase" showToolbar={false} bind:tool={maskTool} bind:brushRadius={maskBrushRadius} bind:brushStrokes={cleanupBrushStrokes} bind:regionX bind:regionY bind:regionW bind:regionH />
                {/if}
              </div>
            {/if}
    ...
    {#if alignerOpen}
      <aside class="aligner-panel">
        <header><span>Frame tools</span><button onclick={()=>alignerOpen=false}>Close</button></header>
        <nav class="aligner-tabs" aria-label="Frame tool sections">
          <button class:active={alignerSection==="align"} onclick={()=>alignerSection="align"}>Align</button>
          <button class:active={alignerSection==="character"} onclick={()=>alignerSection="character"}>Character</button>
          <button class:active={alignerSection==="production"} onclick={()=>alignerSection="production"}>Production</button>
          <button class:active={alignerSection==="regen"} onclick={()=>alignerSection="regen"}>Regen</button>
        </nav>
        {#if alignerSection === "align"}
          <p class="muted">Sposta il frame attivo di 1px. Gli offset restano nei metadati, non nel PNG.</p>
          <label class="check"><input type="checkbox" bind:checked={showAlignGhosts}/> Ghost tutti i frame</label>
          <label class="check"><input type="checkbox" bind:checked={nudgeAllFrames}/> Applica nudge a tutti</label>
          <div class="nudge-grid">
            <button disabled={alignerBusy} onclick={()=>nudgeFrame(0,-1,nudgeAllFrames)}>↑</button>
            <button disabled={alignerBusy} onclick={()=>nudgeFrame(-1,0,nudgeAllFrames)}>←</button>
            <button disabled={alignerBusy} onclick={()=>nudgeFrame(1,0,nudgeAllFrames)}>→</button>
            <button disabled={alignerBusy} onclick={()=>nudgeFrame(0,1,nudgeAllFrames)}>↓</button>
          </div>
          <button class="wide" disabled={alignerBusy} onclick={applyActiveOffsetToAll}>Copia offset attivo su tutti</button>
          <button class="wide" disabled={alignerBusy} onclick={resetActiveFrame}>Reset frame attivo</button>
          <button class="wide" disabled={alignerBusy} onclick={resetAlignment}><RotateCcw size={12}/> Reset auto-align</button>
          {#if sizeContract}<div class="contract"><strong>Size contract</strong>{#if sizeContract.passed}<p class="good">Passed</p>{:else}<ul>{#each sizeContract.violations as violation}<li class:block={violation.blocking}>{violation.message}</li>{/each}</ul>{/if}</div>{/if}
        {:else if alignerSection === "character"}
          <div class="direction-set">
            <strong>Direction set</strong>
            <label>Set<select bind:value={directionSet}><option value="4">4-way</option><option value="8">8-way</option></select></label>
            <div class="facing-grid">{#each directionFacings as facing}<span class:filled={facingIsFilled(facing)} class:current={currentAnimationFacing===facing} class:issue={facingHasContractIssue(facing)} class:pending={facingBadge(facing)==="pending"} class:mirrored={facingBadge(facing)==="mirrored"} class:ai={facingBadge(facing)==="ai"} title={facingMetaFor(facing)?.mirroredFrom?`Mirrored from ${facingMetaFor(facing)?.mirroredFrom}`:facingBadge(facing)}>{facing.toUpperCase()}{#if facingBadge(facing)==="pending"}<small>?</small>{:else if facingBadge(facing)==="mirrored"}<small>M</small>{:else if facingBadge(facing)==="ai"}<small>AI</small>{/if}</span>{/each}</div>
            {#if directionSetNeedsConversation}<p class="warn">Facings mancanti ({estimatePendingFacings().join(", ")}) richiedono una conversazione chat per la generazione AI.</p>{/if}
            <button class="wide" disabled={!animationId||directionSetBusy} onclick={mirrorEastFromWest}>Mirror E from W</button>
            <button class="wide" disabled={!animationId||!worktreeId||directionSetBusy||directionSetActive} onclick={queueDirectionSetJob}>{directionSetActive?"Direction set running…":"Queue direction set"}</button>
            {#if directionSetActive&&directionSetJob}<article class="contract-job"><div><strong>{directionSetJob.stage}</strong><span>{Math.round(directionSetJob.progress*100)}%</span></div><i><b style={`width:${directionSetJob.progress*100}%`}></b></i></article>{/if}
            ...
            <button class="wide" disabled={!worktreeId||directionSetBusy} onclick={refreshCharacterContract}>Check character contract</button>
            {#if characterContract}<p class={characterContract.passed?"good":"warn"}>{characterContract.passed?"Worktree contract passed":`${characterContract.crossFacingViolations?.length ?? 0} cross-facing issue(s)`}</p>{/if}
          </div>
        {:else if alignerSection === "production"}
          <div class="production-tools">
            <strong>Production</strong>
            <label class="check"><input type="checkbox" bind:checked={hardenCleanAlpha}/> Clean alpha</label>
            <label class="check"><input type="checkbox" bind:checked={hardenNormalize}/> Normalize</label>
            <label class="check"><input type="checkbox" bind:checked={hardenSnapGrid}/> Snap grid</label>
            <label class="grid-size">Grid<select bind:value={hardenGridSize}><option value={1}>1px</option><option value={2}>2px</option><option value={4}>4px</option></select></label>
            <label class="check"><input type="checkbox" bind:checked={hardenQuantizePalette}/> Shared palette on harden</label>
            <label class="grid-size">Palette<select bind:value={hardenPaletteColorCount}><option value={8}>8</option><option value={16}>16</option><option value={32}>32</option></select></label>
            <label class="check"><input type="checkbox" bind:checked={hardenQueueRetry}/> Auto contract retry</label>
            <button class="wide primary-production" disabled={!animationId||!frames.length||pipelineBusy} onclick={runHarden}><Hammer size={12}/>{pipelineBusy?"Production…":"Run production harden"}</button>
            <button class="wide" disabled={!worktreeId||paletteBusy||pipelineBusy} onclick={runSharedPalette}>Palette worktree ({hardenPaletteColorCount})</button>
            <button class="wide" disabled={!animationId||!frames.length||pipelineBusy} onclick={runCleanAlpha}><Eraser size={12}/> Clean alpha only</button>
            <button class="wide" disabled={!animationId||!frames.length||pipelineBusy} onclick={runSnapGrid}><Grid3x3 size={12}/> Snap offsets</button>
            {#if lastHardenReport}<p class="muted tiny">Steps: {lastHardenReport.steps.join(" → ")}</p>{/if}
          </div>
          <div class="pipeline-tools">
            <strong>Pipeline</strong>
            <button class="wide" disabled={!animationId||pipelineBusy} onclick={normalizeFrames}><Layers size={12}/> Normalize to anchor</button>
            <button class="wide" disabled={pipelineBusy} onclick={importStrip}><Scissors size={12}/> Import strip</button>
            <button class="wide" disabled={pipelineBusy} onclick={importVideo}><Film size={12}/> Import video</button>
            <button class="wide" disabled={!animationId||pipelineBusy} onclick={()=>applyContractRetry(false)}><RefreshCw size={12}/> Fix contract</button>
            <button class="wide" disabled={!animationId||!conversationId||pipelineBusy||contractRetryActive} onclick={queueAutonomousContractRetry}><Sparkles size={12}/>{contractRetryActive?"Auto-fix running…":"Auto-fix contract"}</button>
            {#if contractRetryActive&&contractRetryJob}<article class="contract-job"><div><strong>{contractRetryJob.stage}</strong><span>{Math.round(contractRetryJob.progress*100)}%</span></div><i><b style={`width:${contractRetryJob.progress*100}%`}></b></i><button class="cancel-job" disabled={pipelineBusy} onclick={cancelContractRetryJob}><X size={10}/>Cancel</button></article>{/if}
            <button class="wide" disabled={!animationId||pipelineBusy} onclick={finalizeRegeneratedStrip}>Finalize AI strip</button>
            <p class="muted tiny">Fix runs normalize/nudge only. Auto-fix queues repair, AI regeneration, import, and re-check in the background.</p>
          </div>
        {:else}
          <div class="region-regen">
            <strong>Regen regione</strong>
            <p class="muted tiny">Disegna una maschera sul frame attivo ({activeFrame + 1}). Richiede chat attiva.</p>
            <div class="mask-mode"><button class:active={maskOverlayMode==="regen"} onclick={()=>{maskOverlayMode="regen"; maskTool="rect";}}>Regen mask</button><button class:active={maskOverlayMode==="erase"} onclick={()=>{maskOverlayMode="erase"; maskTool="erase";}}>Cleanup</button></div>
            <div class="mask-tools">
              <button class:active={maskTool==="rect"} onclick={()=>maskTool="rect"}>Rect</button>
              <button class:active={maskTool==="brush"||maskTool==="erase"} onclick={()=>maskTool=maskOverlayMode==="erase"?"erase":"brush"}>Brush</button>
              <label>Radius<input type="number" min="1" max="64" bind:value={maskBrushRadius}/></label>
              <button onclick={clearMaskOverlay}>Clear</button>
            </div>
            <div class="region-grid"><label>X<input type="number" min="0" bind:value={regionX}/></label><label>Y<input type="number" min="0" bind:value={regionY}/></label><label>W<input type="number" min="1" bind:value={regionW}/></label><label>H<input type="number" min="1" bind:value={regionH}/></label></div>
            {#if maskOverlayMode==="erase"&&currentAsset}<FrameCleanupToolbar assetId={currentAsset.id} strokes={cleanupBrushStrokes} versions={currentAssetVersions} busy={pipelineBusy} onApplied={async()=>{await onAssetsRefresh?.();previewEpoch+=1;await refreshCurrentAssetVersions();}} onError={onError} onNotice={onNotice}/>{/if}
            <label class="region-prompt">Prompt opzionale<textarea rows="2" bind:value={regionPrompt} placeholder="Es. correggi l'artiglio sinistro"></textarea></label>
            <button class="wide" disabled={!animationId||!conversationId||!currentAsset||regionRegenBusy||maskOverlayMode!=="regen"} onclick={queueRegionRegen}><Paintbrush size={12}/>{regionRegenBusy?"Regen…":"Regen region"}</button>
            {#if regionRegenJob&&animationId&&regionRegenJob.targetId===animationId&&["queued","running","analyzing"].includes(regionRegenJob.status)}<article class="contract-job"><div><strong>{regionRegenJob.stage}</strong><span>{Math.round(regionRegenJob.progress*100)}%</span></div><i><b style={`width:${regionRegenJob.progress*100}%`}></b></i></article>{/if}
          </div>
        {/if}
      </aside>
    {/if}
...
<style>
  ...
  .body{flex:1;min-height:0;display:grid;grid-template-columns:190px minmax(0,1fr)}.body.with-quality{grid-template-columns:190px minmax(0,1fr) minmax(285px,300px)}header .status{text-transform:uppercase;font-size:10px}.status.accepted{color:#5ead7b}.status.rejected{color:#cc6863}.status.draft{color:var(--faint)}
  .aligner-panel{border-left:1px solid var(--border);background:var(--sidebar);padding:12px;display:flex;flex-direction:column;gap:10px;overflow:auto;min-width:0}.aligner-panel .tiny{font-size:10px;margin:0}.aligner-panel header{display:flex;justify-content:space-between;align-items:center;font-size:11px;color:var(--muted)}.aligner-panel header button{border:0;background:transparent;color:var(--faint);cursor:pointer}.aligner-tabs{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:4px}.aligner-tabs button{height:26px;border:1px solid var(--border);background:var(--surface);color:var(--faint);border-radius:4px;font:inherit;font-size:9px;cursor:pointer;padding:0 4px}.aligner-tabs button.active{border-color:var(--accent);color:var(--text);background:var(--accent-dim)}.mask-tools{display:flex;gap:6px;align-items:center;flex-wrap:wrap}.mask-tools button{height:24px;border:1px solid var(--border);background:var(--surface);border-radius:4px;color:var(--muted);font:inherit;font-size:10px;padding:0 8px;cursor:pointer}.mask-tools button.active{border-color:var(--accent);color:var(--text)}.mask-tools label{display:flex;align-items:center;gap:4px;font-size:9px;color:var(--faint)}.mask-tools input{width:42px;height:22px;border:1px solid var(--border);background:var(--bg);border-radius:4px;color:var(--text);font:inherit;font-size:10px;padding:0 4px}.nudge-grid{display:grid;grid-template-columns:repeat(3,1fr);gap:6px;max-width:120px}.nudge-grid button{height:30px;border:1px solid var(--border);background:var(--surface);color:var(--text);border-radius:4px;cursor:pointer}.nudge-grid button:nth-child(1){grid-column:2}.nudge-grid button:nth-child(2){grid-column:1;grid-row:2}.nudge-grid button:nth-child(3){grid-column:3;grid-row:2}.nudge-grid button:nth-child(4){grid-column:2;grid-row:2}.aligner-panel .wide{height:28px;border:1px solid var(--border);background:var(--surface);color:var(--muted);border-radius:4px;display:flex;gap:6px;align-items:center;justify-content:center;font:inherit;font-size:11px;cursor:pointer}.contract{border-top:1px solid var(--border);padding-top:10px;font-size:11px;color:var(--muted)}.contract ul{margin:8px 0 0;padding-left:16px}.contract li.block{color:#cc6863}.contract .good{color:#5ead7b;margin:6px 0 0}.animation-list{border-right:1px solid var(--border);background:var(--sidebar);padding:12px 8px;overflow:auto}.label{font-size:10px;color:var(--faint);letter-spacing:.13em;font-weight:700;padding:4px 7px 7px}.templates-label{border-top:1px solid var(--border);margin-top:10px;padding-top:14px}.animation-list>button{width:100%;height:29px;border:0;background:transparent;color:var(--muted);border-radius:4px;display:grid;grid-template-columns:14px minmax(0,1fr) 24px;gap:6px;align-items:center;text-align:left;padding:0 7px;font:inherit;font-size:12px;cursor:pointer}.animation-list>button.active,.animation-list>button:hover{background:var(--selected);color:var(--text)}.animation-list>button.template :global(svg){color:var(--accent)}.animation-list button span{white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.animation-list button small{font-size:9px;color:var(--faint);text-align:right}.animation-list p{font-size:10px;line-height:1.45;color:var(--faint);padding:3px 7px}
