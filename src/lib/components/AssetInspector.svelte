<script lang="ts">
  import { X, ExternalLink, Trash2, Pencil, Check, Image as ImageIcon, Layers3, RotateCcw } from "lucide-svelte";
  import { revealItemInDir } from "@tauri-apps/plugin-opener";
  import { api, assetUrl } from "$lib/api";
  import { errorMessage, type Animation, type Asset, type AssetVersion } from "$lib/types";

  let { asset, animations, onClose, onChanged, onDeleted, onError, onNotice }: {
    asset: Asset; animations: Animation[]; onClose: () => void; onChanged: (asset: Asset) => void; onDeleted: () => void; onError: (message: string) => void; onNotice: (message: string) => void;
  } = $props();
  ...
  let previewVersionId = $state<string>();
  $effect(() => {
    if (!editing) name = asset.name;
    if (loadedAssetId === asset.id) return;
    loadedAssetId = asset.id;
    api.listAssetVersions(asset.id).then(value => { versions=value; previewVersionId=value.find(version=>version.selected)?.id??value[0]?.id; }).catch(() => versions=[]);
  });
  let usedBy = $derived(animations.filter(animation => animation.frames.some(frame => frame.assetId === asset.id)));
  let previewVersion = $derived(versions.find(version => version.id === previewVersionId));
  ...
  async function restoreVersion() {
    if (!previewVersion || previewVersion.selected || !previewVersion.available) return;
    if (!window.confirm(`Restore v${previewVersion.versionNumber} as a new current revision? Your current version will remain in history.`)) return;
    busy = true;
    try {
      const restored = await api.restoreAssetVersion(asset.id, previewVersion.id);
      onChanged(restored);
      versions = await api.listAssetVersions(asset.id);
      previewVersionId = versions.find(version => version.selected)?.id;
      onNotice(`Restored v${previewVersion.versionNumber} as a new revision`);
    } catch (error) { onError(errorMessage(error)); }
    finally { busy = false; }
  }
...
<aside class="inspector">
  ...
  <div class="scroll">
    ...
    <section><h3>VERSIONS</h3>{#if versions.length}<div class="version-list">{#each versions as version}<button class:selected={version.id===previewVersionId} class:unavailable={!version.available} onclick={()=>previewVersionId=version.id}><span>v{version.versionNumber}</span><strong>{version.selected ? "Current" : version.changeKind}</strong><small>{version.available ? new Date(version.createdAt).toLocaleDateString() : "Unavailable"}</small></button>{/each}</div>{#if previewVersion}<div class="version-preview"><div>{#if previewVersion.available}<img src={assetUrl(previewVersion.path)} alt={`Preview of version ${previewVersion.versionNumber}`}/>{:else}<span>Pixels unavailable</span>{/if}</div><p>v{previewVersion.versionNumber} · {previewVersion.width}×{previewVersion.height} · {formatBytes(previewVersion.fileSize)}</p><button class="wide restore" onclick={restoreVersion} disabled={busy||previewVersion.selected||!previewVersion.available}><RotateCcw size={12}/>{previewVersion.selected ? "Current version" : "Restore as new revision"}</button></div>{/if}{:else}<p class="muted">No indexed versions yet.</p>{/if}</section>
...
<style>
  ...
  section{padding:14px;border-bottom:1px solid var(--border)}.name-row{display:flex;justify-content:space-between;align-items:center;gap:6px}.name-row h2{font-size:12px;margin:0;font-weight:600;overflow-wrap:anywhere}.name-row p{font-size:11px;color:var(--faint);margin:3px 0 0;text-transform:capitalize}.name-row input{min-width:0;flex:1;height:27px;border:1px solid var(--accent);background:var(--bg);border-radius:4px;color:var(--text);font:inherit;font-size:12px;padding:0 7px;outline:0}h3{font-size:10px;letter-spacing:.14em;color:var(--faint);margin:0 0 11px}dl{margin:0;display:flex;flex-direction:column;gap:9px}dl div{display:flex;justify-content:space-between;gap:10px;font-size:11px}dt{color:var(--faint)}dd{margin:0;color:var(--muted);text-align:right}.path{font-size:11px;line-height:1.45;color:var(--muted);overflow-wrap:anywhere;margin:0 0 10px}.wide{height:28px;width:100%;border:1px solid var(--border);background:var(--surface);color:var(--muted);border-radius:4px;display:flex;gap:6px;align-items:center;justify-content:center;font:inherit;font-size:11px;cursor:pointer}.wide:hover{color:var(--text);border-color:var(--border-strong)}.used-list{display:flex;flex-direction:column;gap:8px}.used-list div{display:grid;grid-template-columns:13px minmax(0,1fr) auto;align-items:center;gap:5px;font-size:11px;color:var(--muted)}.used-list small{font-size:10px;color:var(--faint)}.version-list{display:flex;flex-direction:column;gap:5px}.version-list>button{width:100%;display:grid;grid-template-columns:24px minmax(0,1fr) auto;gap:6px;align-items:center;padding:6px;border:1px solid var(--border);border-radius:4px;background:transparent;color:var(--muted);font:inherit;text-align:left;cursor:pointer}.version-list>button.selected{border-color:var(--accent);background:var(--accent-dim)}.version-list>button.unavailable{opacity:.55}.version-list span{font-size:10px;color:var(--faint)}.version-list strong{font-size:10px;font-weight:600;text-transform:capitalize}.version-list small{font-size:9px;color:var(--faint)}.version-preview{margin-top:8px}.version-preview>div{height:112px;background:var(--preview);border:1px solid var(--border);border-radius:5px;display:grid;place-items:center;color:var(--faint);font-size:10px}.version-preview img{max-width:88%;max-height:88%;object-fit:contain;image-rendering:pixelated}.version-preview p{font-size:9px;color:var(--faint);margin:6px 0}.version-preview .restore{margin-top:7px}.muted{font-size:11px;line-height:1.5;color:var(--faint);margin:0}.danger{border-bottom:0}.danger button{border:0;background:transparent;color:#c56f6b;font:inherit;font-size:11px;display:flex;align-items:center;gap:7px;padding:4px;cursor:pointer}
</style>
