<script lang="ts">
  ...
  let { workspaces, workspace, worktrees, conversations, selectedWorktreeId, selectedConversationId, runningConversationIds, activeView, onView, onProject, onAddProject, onConversation, onNewConversation, onRenameConversation, onArchiveConversation, onDeleteConversation, onListArchivedConversations, onRestoreConversation, onSettings, onManageProject }: {
    ...
    onView:(view:"chat"|"media"|"skills"|"arts"|"guide")=>void;onProject:(workspace:Workspace)=>void|Promise<void>;onAddProject:()=>void;onConversation:(conversation:Conversation)=>void|Promise<void>;onNewConversation:(worktree?:Worktree)=>void|Promise<void>;onRenameConversation:(conversation:Conversation,title:string)=>void|Promise<void>;onArchiveConversation:(conversation:Conversation)=>void|Promise<void>;onDeleteConversation:(conversation:Conversation)=>void|Promise<void>;onListArchivedConversations:()=>Promise<Conversation[]>;onRestoreConversation:(conversation:Conversation)=>void|Promise<void>;onSettings:()=>void;onManageProject:()=>void;
  }=$props();
  let expandedProjects=$state<string[]>([]);
  let lastAutoExpandedWorkspaceId=$state<string>();
  ...
  let deletingId=$state<string>();
  const primary=[{id:"chat",label:"New Chat",icon:MessageCirclePlus},{id:"media",label:"Media Gallery",icon:GalleryThumbnails},{id:"guide",label:"Guide",icon:BookOpen},{id:"skills",label:"Skills",icon:Blocks},{id:"arts",label:"Arts",icon:Paintbrush}] as const;

  $effect(()=>{
    const id=workspace?.id;
    if(!id||id===lastAutoExpandedWorkspaceId)return;
    lastAutoExpandedWorkspaceId=id;
    if(!expandedProjects.includes(id))expandedProjects=[...expandedProjects,id];
  });
              ...
              <div class="worktree">
                {#each chatsFor(worktree) as conversation}
                  <div class="chat" class:selected={conversation.id===selectedConversationId}>
                    ...
                    <div class="chat-actions"><button onclick={()=>renameTarget=conversation} title="Rename"><PenLine size={11}/></button><button onclick={()=>onArchiveConversation(conversation)} title="Archive"><Archive size={11}/></button><button class="danger" onclick={()=>onDeleteConversation(conversation)} title="Delete permanently"><Trash2 size={11}/></button></div>
                  </div>
                {/each}
              </div>
            {/each}
