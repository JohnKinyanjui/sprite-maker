<script lang="ts">
  import { ArchiveRestore, LoaderCircle, Trash2, X } from "lucide-svelte";
  import type { Conversation, Worktree } from "$lib/types";

  let { conversations, worktrees, loading = false, restoringId, deletingId, onRestore, onDelete, onClose }: {
...
<div class="backdrop" role="presentation" onclick={(event)=>event.target===event.currentTarget&&onClose()}>
  <div class="dialog" role="dialog" aria-modal="true" aria-labelledby="archived-chats-title">
    <header>
      <div><p>PROJECT HISTORY</p><h2 id="archived-chats-title">Archived chats</h2><span>Restore a chat to continue, or delete it permanently.</span></div>
      ...
      {:else}
        {#each conversations as conversation}
          <article>
            ...
            <div class="actions">
              <button onclick={()=>onRestore(conversation)} disabled={Boolean(restoringId || deletingId)} aria-label={`Restore ${conversation.title}`}><ArchiveRestore size={14}/>{restoringId===conversation.id ? "Restoring…" : "Restore"}</button>
              <button class="danger" onclick={()=>onDelete(conversation)} disabled={Boolean(restoringId || deletingId)} aria-label={`Delete ${conversation.title}`}><Trash2 size={14}/>{deletingId===conversation.id ? "Deleting…" : "Delete"}</button>
            </div>
          </article>
        {/each}
      {/if}
