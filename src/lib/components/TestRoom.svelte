<script lang="ts">
  ...
  let currentFrame = $derived(activeAnimation?.frames[activeFrame]);
  let currentAsset = $derived(assets.find(asset => asset.id === currentFrame?.assetId));
  let frameOffsetX = $derived(currentFrame?.offsetX ?? 0);
  let frameOffsetY = $derived(currentFrame?.offsetY ?? 0);
      ...
      <div class="room-shell">
        <div class="room" class:grid style={`transform:scale(${zoom})`}>
          ...
          <div class="character" style={`transform:translate(${x}px,${y}px) scale(${spriteScale})`}>
            {#if currentAsset}<img src={assetUrl(currentAsset.path)} alt="Playable character" style={`transform:translate(calc(-50% + ${frameOffsetX}px), ${frameOffsetY}px)`}/>{:else}<div class="placeholder"><Gamepad2 size={17}/></div>{/if}
