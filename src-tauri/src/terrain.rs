...
fn cell_has_pixels(
    ...
    (start_y..start_y + input.tile_height).any(|y| {
        (start_x..start_x + input.tile_width).any(|x| image.get_pixel(x, y).0[3] != 0)
    })
}

fn terrain_peering_bits(role: &str) -> Option<Vec<&'static str>> {
    if let Some(mask) = role.strip_prefix("blob_").and_then(|value| value.parse::<u8>().ok()) {
    ...
    for (index, mask) in masks.into_iter().enumerate() {
        ...
        for y in 0..input.tile_height {
            for x in 0..input.tile_width {
                ...
                let source_x = input.margin_x
                    + source_column * (input.tile_width + input.separation_x)
                    + x;
                let source_y = input.margin_y
                    + source_row * (input.tile_height + input.separation_y)
                    + y;
                output.put_pixel(
...
pub fn export_godot_tileset(
    ...
    let (export_image, export_input, export_layout, export_occupied) = if mode == "blob_47" {
        ...
        let generated_occupied = generated_input
            ...
            .collect::<Vec<_>>();
        (generated, generated_input, generated_layout, generated_occupied)
    } else {
...
mod tests {
    use super::{
        blob_masks, blob_rules, expand_blob_atlas, godot_resource, grid_layout, is_blob_mask,
        slug, validate_terrain_rules, GridLayout, BLOB_COLUMNS, TERRAIN_RULE_ROLES,
    };
