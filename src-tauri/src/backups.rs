...
fn safe_slug(value: &str) -> String {
    ...
    if slug.is_empty() { "sprite-project".into() } else { slug.into() }
}
    ...
    for entry in std::fs::read_dir(source)? {
        ...
        if metadata.file_type().is_symlink() {
            return Err(CommandError::new(
                "backup_symlink",
                format!("Backups do not follow symbolic links: {}", source_path.display()),
            ));
        }
        if metadata.is_dir() {
...
fn workspace_record(state: &AppState, project_id: &str) -> CommandResult<Workspace> {
    ...
    connection
        .query_row(
            ...
            [project_id],
            |row| Ok(Workspace { id: row.get(0)?, name: row.get(1)?, path: row.get(2)?, created_at: row.get(3)?, last_opened_at: row.get(4)? }),
        )
        .optional()?
        .ok_or_else(|| CommandError::new("workspace_not_found", "Workspace is no longer registered"))
}

fn create_database_snapshot(state: &AppState, destination: &Path) -> CommandResult<()> {
    let source = state
        .db
        .lock()
        .map_err(|_| CommandError::new("database_locked", "Database lock was poisoned"))?;
...
fn create_project_backup_internal(
    ...
    let label = format!("{}-{}.sprite-studio-backup", safe_slug(&workspace.name), timestamp.format("%Y%m%d-%H%M%S"));
    let mut final_path = destination_directory.join(&label);
    if final_path.exists() {
        let suffix = Uuid::new_v4().simple().to_string();
        final_path = destination_directory.join(format!("{}-{}.sprite-studio-backup", label.trim_end_matches(".sprite-studio-backup"), &suffix[..8]));
    }
    let staging = destination_directory.join(format!(".sprite-studio-backup-{}.incomplete", Uuid::new_v4()));
    std::fs::create_dir_all(&staging)?;
    let result = (|| -> CommandResult<ProjectBackup> {
        let (file_count, total_bytes) = copy_tree(&root, &staging.join("workspace"))?;
        create_database_snapshot(state, &staging.join("metadata.sqlite3"))?;
        ...
        std::fs::write(
            ...
            serde_json::to_vec_pretty(&backup).map_err(|error| CommandError::new("serialization_error", error.to_string()))?,
        )?;
...
fn read_manifest(backup_path: &Path) -> CommandResult<ProjectBackup> {
    ...
    {
        return Err(CommandError::new("invalid_backup", "This is not a complete Sprite Studio project backup"));
    }
    Ok(manifest)
}
...
fn copy_project_rows(
    ...
    if !inserted {
        return Err(CommandError::new("restore_failed", "Restored project metadata could not be verified"));
    }
    Ok(())
}
...
pub fn import_project_backup(
    ...
    let existing: bool = state.db.lock().map_err(|_| CommandError::new("database_locked", "Database lock was poisoned"))?.query_row(
    ...
    if destination.parent().is_none() || destination.components().count() < 3 {
        return Err(CommandError::new("unsafe_restore", "Choose a dedicated project folder"));
    }
    if std::fs::read_dir(&destination)?.next().is_some() {
        return Err(CommandError::new("restore_destination_not_empty", "Choose an empty folder for the restored project"));
    }
    if destination.starts_with(&backup_path) || backup_path.starts_with(&destination) {
        return Err(CommandError::new("unsafe_restore", "The restored project and its backup must be separate folders"));
    }
    copy_tree(&backup_path.join("workspace"), &destination)?;
    let import_result = (|| -> CommandResult<()> {
        let mut connection = state.db.lock().map_err(|_| CommandError::new("database_locked", "Database lock was poisoned"))?;
        connection.execute("ATTACH DATABASE ?1 AS project_backup", [backup_path.join("metadata.sqlite3").to_string_lossy().as_ref()])?;
        let insert_result = (|| -> CommandResult<()> {
            let transaction = connection.transaction()?;
            copy_project_rows(&transaction, &manifest.project_id, &manifest.source_path, destination.to_string_lossy().as_ref())?;
            transaction.commit()?;
            Ok(())
        })();
        let detach_result = connection.execute_batch("DETACH DATABASE project_backup").map_err(CommandError::from);
        insert_result?;
        detach_result
    })();
...
fn restore_project_backup_internal(
    ...
    if root.parent().is_none() || root.components().count() < 3 {
        return Err(CommandError::new("unsafe_restore", "Refusing to replace a broad filesystem path"));
    }
    let backup_path = backup_path.canonicalize()?;
    let manifest = read_manifest(&backup_path)?;
    if manifest.project_id != project_id {
        return Err(CommandError::new("wrong_project_backup", "Choose a backup created from this project"));
    }
    ...
    if running {
        return Err(CommandError::new("project_busy", "Stop active chat generation before restoring a backup"));
    }
    let safety_directory = root.parent().expect("validated parent").join(".sprite-studio-safety-backups");
    create_project_backup_internal(project_id, &safety_directory, state)?;
    let staging = root.parent().expect("validated parent").join(format!(".sprite-studio-restore-{}", Uuid::new_v4()));
    let previous = root.parent().expect("validated parent").join(format!(".sprite-studio-previous-{}", Uuid::new_v4()));
    ...
    let restore_result = (|| -> CommandResult<()> {
        ...
        let import_result = (|| -> CommandResult<()> {
            ...
            copy_project_rows(&transaction, project_id, &manifest.source_path, root.to_string_lossy().as_ref())?;
            transaction.commit()?;
            Ok(())
        })();
        let detach_result = connection.execute_batch("DETACH DATABASE project_backup").map_err(CommandError::from);
...
mod tests {
    ...
    fn project_backup_restores_files_and_metadata_after_making_a_safety_copy() {
        ...
        let state = AppState { db: Mutex::new(connection), cancellers: Mutex::new(HashMap::new()) };

        let backup = create_project_backup_internal("project", &backups, &state).expect("backup creates");
        std::fs::write(project.join("assets/characters/hero.txt"), "after").expect("project mutates");
        state.db.lock().expect("database lock").execute(
            "UPDATE conversations SET title='After backup' WHERE id='chat'",
            [],
        ).expect("chat mutates");

        restore_project_backup_internal("project", std::path::Path::new(&backup.backup_path), &state).expect("backup restores");

        assert_eq!(std::fs::read_to_string(project.join("assets/characters/hero.txt")).expect("file reads"), "before");
        let title: String = state.db.lock().expect("database lock").query_row(
