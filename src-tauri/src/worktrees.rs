...
pub fn update_worktree(
    ...
    if kind == "general" {
        return Err(CommandError::new(
            "protected_worktree",
            "The General worktree type cannot be assigned or changed",
        ));
    }
    ...
    let changed = connection.execute(
        "UPDATE worktrees SET name = ?1, kind = ?2, description = ?3, updated_at = ?4 WHERE id = ?5",
        params![name, kind, description, now, id],
    )?;
    if changed == 0 {
        return Err(CommandError::new(
            "worktree_not_found",
...
mod tests {
    ...
    #[test]
    fn deleting_a_worktree_preserves_chats_messages_and_assets_in_general() {
        ...
        let general_asset_link: i64 = connection.query_row(
            "SELECT COUNT(*) FROM asset_worktrees WHERE asset_id='a1' AND worktree_id='general'",
            [],
            |row| row.get(0),
        ).expect("asset link count should work");
        assert_eq!(general_asset_link, 1);
        assert_eq!(
