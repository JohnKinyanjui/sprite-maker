...
If the first candidate fails, silently repair its master, rig, mask, layer order, anchor, weighting, timing, or optional polish and render exactly one replacement. Apply the same gates to the replacement. Publish only an accepted candidate; if the replacement also fails, stop with a terminal error instead of exposing broken art.
