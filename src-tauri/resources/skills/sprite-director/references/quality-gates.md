...
9. **Animation loop:** validate the cycle headlessly (strict rig validator plus a contact-sheet inspection that treats last→first as an ordinary adjacent transition); never open a browser or interactive preview. Feet and pivot do not drift, motion arcs are intentional, and the final-to-first transition is no worse than an ordinary adjacent transition.
