...
- Generate one coherent pack of 4–12 **static, individually usable game assets**. If the user gives a count, honor it within that range; otherwise choose 6.
...
2. Use ImageGen to create one high-quality contact sheet with isolated, non-overlapping items on a uniform removable background. This is the visual master for consistency.
...
Report the pack name, item count, shared canvas, style, categories, and the pack manifest path. Link the individual assets, but do not describe them as frames.
